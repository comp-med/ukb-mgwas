...
#SBATCH --output=slurm_logs/males_step2_%j.out
#SBATCH --chdir=
#SBATCH -J RegStep2_males
...
tmpdir=tmpmales/
regenie_step1_out=output/preds/males/step1_pred.list
out=output/gwas/males/chr${chr}
...
varqc=variant_qc/output/ukb_imp_chr${chr}_qced.txt
cat ${varqc} | awk -v chr=${chr} '{if(NR != 1 && $3 == chr) print $2}' > input/varqc_lists/males/variants_chr${chr}.list
varqc_chr=input/varqc_lists/males/variants_chr${chr}.list
