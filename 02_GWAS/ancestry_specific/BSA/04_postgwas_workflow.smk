...
GWSIG_THRESHOLD = 9.7 # -log10(5e-8 / 251)
...
rule all:
    input:
        ...
        expand("output/regions_interest_thre4/regions_{phenotype}.txt", phenotype=PHENOTYPES),
        expand("output/sentinels/sentinels_{phenotype}.txt", phenotype=PHENOTYPES)

rule collate:
    input: infiles = expand("output/gwas/chr{chr}_{{phenotype}}.regenie.gz", chr=CHROMOSOMES)
...
rule ldsc:
    ...
    shell:
        ...
        /sc-projects/sc-proj-computational-medicine/programs/ldsc/munge_sumstats.py \
            ...
            --chunksize 500000 \
            --merge-alleles /sc-projects/sc-proj-computational-medicine/programs/ldsc/w_hm3.snplist

        /sc-projects/sc-proj-computational-medicine/programs/ldsc/ldsc.py \
            --h2 output/ld_score_regression/{wildcards.phenotype}.sumstats.gz \
            --ref-ld-chr /sc-projects/sc-proj-computational-medicine/programs/ldsc/eur_w_ld_chr/ \
            --w-ld-chr /sc-projects/sc-proj-computational-medicine/programs/ldsc/eur_w_ld_chr/ \
...
rule get_regions_interest:
    ...
    shell:
        ...
        /sc-projects/sc-proj-computational-medicine/programs/bedtools merge -i stdin > {output.regions_of_interest}
        """

rule get_regions_interest_lenient:
    input: summarystats = "output/gwas_collated/gwas_{phenotype}_allchr.txt.gz"
    output: regions_of_interest = "output/regions_interest_thre4/regions_{phenotype}.txt"
    params:
        threshold = 3
    resources:
        mem='10G',
    ...
    shell:
        ...
        zcat {input.summarystats} | \
        awk '{{if(NR == 1) print $0; else if (NR > 1 && ($13+0) > {params.threshold}) print $0}}'| \
        sed '1d'  | \
        awk '{{print $1,$2,$2}}' OFS="\t"  | \
        sort -k1,1 -k2,2n | \
        awk '{{if($1+0.0==6+0 && $2+0 >= 25500000+500000 && $2+0 <= 34000000-500000) {{print $1,25500000,34000000}} \
...
rule define_regional_sentinels:
    input:
        summarystats = "output/gwas_collated/gwas_{phenotype}_allchr.txt.gz",
